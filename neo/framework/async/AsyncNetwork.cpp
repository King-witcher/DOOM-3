...
void idAsyncNetwork::WriteUserCmdDelta( idBitMsg &msg, const usercmd_t &cmd, const usercmd_t *base ) {
	if ( base ) {
		...
		msg.WriteDeltaShort( base->buttons, cmd.buttons );	// Quake4: buttons is a short
	...
	msg.WriteShort( cmd.buttons );	// Quake4: buttons is a short
...
void idAsyncNetwork::ReadUserCmdDelta( const idBitMsg &msg, usercmd_t &cmd, const usercmd_t *base ) {
	...
	if ( base ) {
		...
		cmd.buttons = msg.ReadDeltaShort( base->buttons );	// Quake4: buttons is a short
    ...
    cmd.buttons = msg.ReadShort();	// Quake4: buttons is a short
